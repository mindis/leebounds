...
list_of_all_controls<-setdiff(colnames(prepared_data), c("person_id","household_id","treatment", "ohp_all_ever_survey",
                                                         all_names, "weight_12m","sample_12m","sample_12m_resp","group_id", setdiff(grep("ddd",colnames(prepared_data_sample_12m),value=TRUE),
                                                         stratification_controls)))
...
for (outcome_name in numeric_health_outcomes) {
  ...
  myres<- foreach(b=1:B, .combine = 'cbind',.packages = c("tidyverse","AER","miceadds","hdm"))  %dopar% {
    ...
    ans<-tryCatch(bootstrap_mean_itt_late_ml(b,
                                    ...
                                    selection_function_for_threshold=glm,
                                    selection_function_for_threshold_name="glm",
                                    stratification_controls= stratification_controls[c(1:7,9)],
                                    selection_function_point_mass=rlassologit,
                                    selection_function_point_mass_name="rlassologit",
                                    ...
                                    selected_inds=NULL,
                                    max_p0_hat=1.001001,
                                    point_mass=FALSE, trimming_value=0,weight_name="weight_12m"),error=function(e) NULL)
    #tryCatch(
  }
  cluster_robust_se[[outcome_name]]<-apply(myres,1,sd)
...
for (outcome_name in discrete_outcome_names) {
  ...
  myres<- foreach(b=1:B, .combine = 'cbind',.packages = c("tidyverse","AER","miceadds","hdm"))  %dopar% {
    ...
    ans<-bootstrap_mean_itt_late_ml(b,
                                             ...
                                             weight_name="weight_12m",point_mass_cutoff=0.25,
                                             max_p0_hat=1.000001)
...
colnames(ml_trim_sd)<-c(utilization_outcomes, discrete_outcome_names,numeric_health_outcomes)
...
write.csv(ml_trim_sd,paste0(my_path,"/OHIE/STEP3_ML/csv/sd_ml_trim_numeric_health.csv"))
save.image(paste0(my_path,"/OHIE/STEP3_ML/draft/ml_trim_boot.RData"))
