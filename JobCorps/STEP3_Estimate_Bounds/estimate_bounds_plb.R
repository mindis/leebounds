...
orthoestimates_plb2<-matrix(0,2,length(selected_weeks))
...
for (i in 1:6) {
  ...
  res<-summary_subjects_positive_lower_bound(leedata_cov_total=leedata_cov[,c("treat","selection","outcome",selected_vars)],
 ...
 is_positive_list[[i]]<-res$is_positive
}
...
ps_any_week<-(is_positive_list[[1]]+is_positive_list[[2]]+is_positive_list[[3]]+is_positive_list[[4]]+is_positive_list[[5]]+is_positive_list[[6]])>0
  ...
  leebounds_ortho_result3<-second_stage_wrapper(leedata=leedata_cov[  ps_any_week ,],
                                                inds_helps=inds_helps[ps_any_week],
                                                s.hat= leebounds_ortho_result$s.hat[  ps_any_week,],
                                                y.hat=leebounds_ortho_result$y.hat[ps_any_week,],
                                                weight=Lee_data$DSGN_WGT.y[ps_any_week],ortho=TRUE,c_quant=0) 
  orthoestimates_plb2[,i]<-GetBounds(leebounds_ortho_result3)
  
  estimated_orthobounds_bb<-main_bb(leedata_cov[  ps_any_week ,],N_rep=N_rep,function_name=second_stage_wrapper,
                                    y.hat= leebounds_ortho_result$y.hat[  ps_any_week ,],s.hat=leebounds_ortho_result$s.hat[  ps_any_week ,],
                                    inds_helps=leebounds_ortho_result$inds_helps[  ps_any_week ],weight=Lee_data$DSGN_WGT.y[  ps_any_week ])
  ...
  IM_ortho_plb[,i]<-imbens_manski(estimated_orthobounds_bb, orthoestimates_plb2[,i], ci_alpha=ci_alpha)
}

estimates_table<-rbind(orthoestimates_plb2)
...
write.csv(IM_table,paste0(my_path,"JobCorps/STEP5_Print_Tables/csv/Table1_Col5_IM_plb.csv"))
