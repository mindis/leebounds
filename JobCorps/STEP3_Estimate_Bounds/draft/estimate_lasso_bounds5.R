...
selected_covs_selection[[1]]<-c("treat:PERS_INC3","treat:MOS_AFDC")
selected_covs_selection[[2]]<-c("treat:EARN_YR" ,"treat:R_HOME1", "FEMALE" ,       "BLACK","AGE" )
selected_covs_outcome[[2]]<-c("AGE","FEMALE","BLACK")
selected_covs_selection[[3]]<-c("treat:EARN_YR" ,"treat:R_HOME1", "FEMALE" ,       "BLACK","AGE" )
...
selected_covs_selection[[5]]<-c( "treat:TYPEWORR5","treat:R_HOME1","treat:DRG_SUMP2","treat:TYPEJOBB9")
selected_covs_outcome[[5]]<-c("FEMALE","BLACK","IMPRCNTR4")

selected_covs_selection[[6]]<-c("treat:DRG_SUMP2","treat:TYPEJOBB9")
...
united_names<-setdiff( c(setdiff(covs[[i]],grep("treat:",covs[[i]],value=TRUE)),unlist(strsplit(grep("treat:",covs[[i]],value=TRUE),"treat:"))),   c("","treat","(Intercept)") )
...
penalty[6]<-550
  ...
  for (i in 1:6) {
    ...
    if (i>=4) {
      ...
      form_nonmonotone_ss<-as.formula(paste0("selection~(",paste0("treat",collapse="+"), ")*(",paste0(selected_names,collapse="+"),")" ))
    ...
    glm.fit<-estimate_selection(form=form_nonmonotone_ss,leedata=leedata_cov,selection_function_name = "rlassologit",
                                names_to_include=c("R_HOME1","R_HOME1:R_COMM1","R_GETGED1"),
    ...
    leebounds_ortho_result<-ortho_leebounds(leedata_cov=leedata_cov,s.hat= s.hat,
                                            ...
                                            variables_for_outcome=c("FEMALE","BLACK","EARN_YR","WKEARNR"),min_wage=min_wage,
    ...
    #,"EARN_YR","HGC","MONINED" , "NUMBJOBS" , "WORK271"
