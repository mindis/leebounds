...
### load data
sink(paste0(my_path,"/JobCorps/STEP3_MISC/Figure4.log"))
print ("Loading data ...")
...
for (k in 1:length(phis)) {
  ...
  estimated_orthobounds_bb[,,k]<-t(weighted_bb(leedata_cov,B=Nboot,function_name=second_stage_wrapper,
                                               y.hat=res$y.hat,s.hat=res$s.hat,inds_helps=res$inds_helps))
...
mean(leedata_cov1$outcome[leedata_cov2$selection==1 & leedata_cov1$selection==1 & inds_helps_helps])
print (paste0("Estimated untreated log (wage) for week ", week2))
mean(leedata_cov2$outcome[leedata_cov2$selection==1 & leedata_cov1$selection==1 & inds_helps_helps])
print (paste0("Estimated untreated log (wage) growth from  ", week1," to ", week2))
mean(leedata_cov2$outcome[leedata_cov2$selection==1 & leedata_cov1$selection==1 & inds_helps_helps])-mean(leedata_cov1$outcome[leedata_cov2$selection==1 & leedata_cov1$selection==1 & inds_helps_helps])
sink(file=NULL)
closeAllConnections()