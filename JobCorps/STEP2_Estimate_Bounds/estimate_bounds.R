...
for (i in 1:length(selected_weeks)) {
  ...
  ### Adding covariates
  glm.fit<-glm(form=form_monotone,data= leedata_cov[,setdiff(colnames(leedata_cov),"outcome")],family="binomial")
  ...
  res<-ortho_leebounds(leedata_cov,    
                       ...
                       variables_for_outcome=baseline_varnames,sort_quantiles=TRUE,weights=NULL)
